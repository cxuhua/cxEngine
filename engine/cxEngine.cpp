...
void cxEngine::Run()
{
    ...
    while(init){
        ...
        glLineWidth(1.0f);
        {
            cxPoint2F ps[1500];
            cxColor4F cs[1500];
            for(cxInt i=0; i < 1500 ; i++){
                cs[i] = color;
                cxPoint2F &pv = ps[i];
                pv.x = (winsize.w/1500) * i - winsize.w/2.0f;
                pv.y = CX_RAND_11f() * 50;
            }
            
            glEnableVertexAttribArray(cxVertexAttribPosition);
            glVertexAttribPointer(cxVertexAttribPosition, 2, GL_FLOAT, GL_FALSE, sizeof(cxPoint2F), ps);
